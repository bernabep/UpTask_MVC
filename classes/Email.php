...
class Email
{
    ...
    { 
        ...
        try {
            ...
            //Recipients
            $mail->setFrom('bpandofdesarrollador@gmail.com', 'Admin');
            ...
            $mail->Body = $contenido;
            $mail->AltBody = 'Por favor, copie el siguiente enlace en su navegador para confirmar su cuenta: http://localhost:3000/confirmar?token=' . $this->token;
            $respuesta = $mail->send();
    ...
    public function recuperar_cuenta(){ 
        ...
        try {
            ...
            $contenido .= '<p>Presiona aquí: <a href="http://localhost:3000/reestablecer?token=' . $this->token . '">Cambiar Contraseña</a></p>';
            $contenido .= '<p>Si tu no has solicitado recuperar la contraseña puedes ignorar este mensaje</p>';
            $mail->Body = $contenido;
            $mail->AltBody = 'Por favor, copie el siguiente enlace en su navegador para recuperar la contarseña de su cuenta: http://localhost:3000/confirmar?token=' . $this->token;
            $respuesta = $mail->send();
